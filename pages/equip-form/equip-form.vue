...
onLoad((query) => {
  if (query.id) {
    const item = store.getEquipById(Number(query.id))
    if (item) {
...
const typeOptions = [
  { key: 'racket', label: '球拍', icon: '🏸' },
  { key: 'shoes', label: '球鞋', icon: '👟' },
  { key: 'bag', label: '球包', icon: '🎒' },
  { key: 'shuttle', label: '羽毛球', icon: '🪶' },
...
function handleSubmit() {
  ...
  if (isEditMode.value) {
    ...
    store.update(editingId.value, equip)

    const purchaseExpenses = expenseStore.getByEquipId(editingId.value).filter(e => e.category === 'equipment')
    if (purchaseExpenses.length > 0) {
      expenseStore.update(purchaseExpenses[0].id, {
        ...
        <view class="selector-trigger" @tap="openModelPicker">
          <view class="selector-value">
            ...
            <text :class="form.model ? 'selector-text' : 'selector-placeholder'">
              {{ form.model || (form.brand ? '选择型号' : '请先选择品牌') }}
            </text>
          </view>
          <text class="selector-arrow">▾</text>
        </view>
